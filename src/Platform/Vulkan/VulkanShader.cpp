...
namespace Broccoli {
	...
	VulkanShader::VulkanShader(const std::string& filePath, VkShaderStageFlagBits stageFlags) : stageFlags(stageFlags), filePath(filePath)
	{
		...
		for (size_t set = 0; set < shaderDescriptorSets.size(); set++)
		{
			...
			if (shaderDescriptorSet.samplerDescriptors.layoutBindings.size() > 0)
			{
				createDescriptorSetLayout(logicalDevice, shaderDescriptorSet.samplerDescriptors.descriptorSetLayout, shaderDescriptorSet.samplerDescriptors.layoutBindings);
			}
